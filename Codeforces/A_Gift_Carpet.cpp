...
void solve(){
    int n, m;
        ...
        for (int vCol = 0; vCol < m; ++vCol) {
            for (int iCol = vCol + 1; iCol < m; ++iCol) {
                for (int kCol = iCol + 1; kCol < m; ++kCol) {
                    for (int aCol = kCol + 1; aCol < m; ++aCol) {
                        bool vFound = false, iFound = false, kFound = false, aFound = false;

                        for (int row = 0; row < n; ++row) {
                            if (matrix[row][vCol] == 'v') {
                                vFound = true;
                            }
                            if (matrix[row][iCol] == 'i') {
                                iFound = true;
                            }
                            if (matrix[row][kCol] == 'k') {
                                kFound = true;
                            }
                            if (matrix[row][aCol] == 'a') {
                                aFound = true;
                            }
                        }
