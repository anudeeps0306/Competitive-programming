...
void solve() {
    int numOfTestCases;
    cin >> numOfTestCases;
    int totalTestCases = numOfTestCases;
    ...
    for (int i = 0; i < numOfTestCases; i++) {
        ...
        valuesPairs.push_back({max(values[0], values[1]), min(values[0], values[1])});
    }

    sortAndReversePairs(valuesPairs);

    int minimum = INT_MAX;
    int sum = 0;
    int size = valuesPairs.size();

    if (totalTestCases == 1) {
        cout << valuesPairs[0].second << endl;
        return;
    }

    for (int i = size - 2; i >= 0; i--) {
        sum += valuesPairs[i].first;
        minimum = min(minimum, valuesPairs[i].second);
    }

    cout << sum + min(minimum, valuesPairs[size - 1].second) << endl;
}
