...
typedef vector<pii> vector_pair;
...
void solve(){
     int n;
        ...
        for (auto itr : m)
        {
            vector_pair.push_back({itr.first, itr.second});
        }
        ...
        if (ans == vector_pair[0].second)
        {
            cout << s << endl;
        }
        else
        {
            int g = vector_pair[0].second - ans + 1;
            int d = 0;
            string s2 = "";
            s2 = s2 + vector_pair[0].first;
            for (int i = 0; i < n; i++){
                if (s[i] == vector_pair[0].first){
                    d++;
                }
                if (d == vector_pair[0].second)
                {
                    d++;
                    continue;
                }
