...
void solve() {
    int x;
    cin >> x;
    vector<int> operations = {x};

    int power = binarySearchPower(x);
    ...
    int remaining = x - current_power, temp = x,j = 0,factor = 1;
    
    while (j < 31) {
        if ((factor & remaining) != 0) {
            operations.push_back(temp - factor);
            temp -= factor;
        }
        factor *= 2;
        j++;
    ...
    while (current_power > 1) {
        current_power >>= 1;
        operations.push_back(current_power);
    }
